...
@php
    $fieldNames = [
        ...
        'interactions_needed' => 'Interactions Needed per Sale',
        'reach_to_interaction_percentage' => 'Reach Needed per Interaction',
    ];
@endphp
      ...
      <div class="card-header d-flex align-items-center justify-content-between">
        <div>
          <h5 class="mb-1">
            ...
            <span class="badge badge-key">#{{ $conversionRate->id }}</span>
          </h5>
      ...
      <div class="card-body">
        ...
        <div class="table-responsive">
          <table id="lastModifiedTable" class="table table-sm align-middle mb-4">
            ...
            <tbody>
              @foreach ($conversionRate->getAttributes() as $key => $newValue)
                @php
                    // decode + display (new)
                      ...
                      <tbody>
                        @foreach ($log->new_data as $key => $newVal)
                          @php
                              ...
                              // map relations for this log row (using values inside $log)
                              $label = $fieldNames[$key] ?? ucfirst(str_replace('_',' ',$key));
                              if ($key === 'user_id') {
...
<script>
  $(function () {
    ...
    $('#exportLastExcel').on('click', function(e){
      ...
      XLSX.writeFile(wb, 'sales_strategy_last_modified.xlsx');
    });
    ...
    $('#exportLastPDF').on('click', function(e){
      ...
      doc.text('Sales Strategy — Last Modified', 40, 40);
      doc.autoTable({ html:'#lastModifiedTable', startY:60, styles:{fontSize:8} });
      doc.save('sales_strategy_last_modified.pdf');
    });
    ...
    $('#exportLogsPDF').on('click', function(e){
      ...
      const doc = new jsPDF('p','pt');
      let y = 40;
      doc.text('Sales Strategy — Change Logs (expanded)', 40, y);
      y += 20;
      ...
      setTimeout(function(){
        ...
        doc.save('sales_strategy_change_logs.pdf');
...
{{-- Bootstrap JS لو مش مضمّن في الـ layout --}}
