...
<div class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <h1 class="text-2xl font-bold mb-4">Sales Strategy Details #{{ $conversionRate->id }}</h1>
                ...
                @foreach ($conversionRate->getAttributes() as $key => $newValue)
                    ...
                    @endphp
                      ...
                      <tr class="transition-colors @if($isDifferent) bg-yellow-50 hover:bg-yellow-100 @else hover:bg-gray-50 @endif cursor-pointer"
                        ...
                        <td class="border px-4 py-2 font-medium break-words max-w-xs"> {{ $fieldNames[$key] ?? ucfirst(str_replace('_', ' ', $key)) }}</td>
                        @php
                                ...
                                @foreach ($log->new_data as $key => $newVal)
                                    ...
                                    @if ($displayNew != $displayOld)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="border px-4 py-2">{{  $fieldNames[$key] ?? ucfirst(str_replace('_', ' ', $key))}}</td>
                                            @php
                                            $oldVal = $oldData[$key] ?? null;
