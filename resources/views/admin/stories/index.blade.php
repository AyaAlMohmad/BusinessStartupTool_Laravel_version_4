...
<style>
    ...
    .cell-scroll{max-height:160px;overflow:auto}
        ...
        <div class="card border shadow-xs mb-4">
            <div class="card-header border-bottom pb-0">
                <div class="d-sm-flex align-items-center mb-3">
                    <div>
                        <h6 class="font-weight-semibold text-lg mb-0">Stories List</h6>
                        <p class="text-sm mb-sm-0">Manage user-submitted stories</p>
                    </div>
                    ...
                    <div class="ms-auto d-flex">
                        ...
                        <div class="btn-group ms-2">
                            <a href="{{ route('admin.stories.analysis') }}" class="btn btn-sm btn-dark">
            ...
            <div class="card-body px-0 py-0">
                    ...
                    <table class="table align-items-center justify-content-center mb-0">
                        ...
                        <tbody>
                            @foreach ($stories as $story)
                                <tr>
                                    <td><p class="text-sm mb-0">{{ $story->id }}</p></td>
                                    <td><p class="text-sm mb-0">{{ $story->user->name ?? 'N/A' }}</p></td>
                                    <td>
                                        <div class="cell-scroll">
                                            @if(is_array($story->title))
                                                {{ implode(', ', $story->title) }}
                                            @else
                                    ...
                                    <td>
                                        <div class="cell-scroll">
                                            @if(is_array($story->country))
                                                {{ implode(', ', $story->country) }}
                                            @else
                                                {{ $story->country ?? 'N/A' }}
                                            @endif
                                        </div>
                                    ...
                                    <td class="align-middle text-center">
                                        <div class="d-flex justify-content-center">
                                            ...
                                            <a href="{{ route('admin.stories.show', $story->id) }}"
                                               ...
                                               data-bs-toggle="tooltip" data-bs-title="View Story">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <!-- Delete -->
                                            <form action="{{ route('admin.stories.destroy', $story->id) }}" method="POST" class="m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                        ...
                                                        data-bs-toggle="tooltip" data-bs-title="Delete Story">
                ...
                <div class="border-top py-3 px-3 d-flex align-items-center">
                    @if ($stories->previousPageUrl())
                        <a href="{{ $stories->previousPageUrl() }}" class="btn btn-sm btn-white d-sm-block d-none mb-0">Previous</a>
                    @else
                    ...
                    <nav aria-label="..." class="ms-auto">
                        <ul class="pagination pagination-light mb-0">
                            @foreach ($stories->getUrlRange(1, $stories->lastPage()) as $page => $url)
                                <li class="page-item {{ $stories->currentPage() == $page ? 'active' : '' }}">
                    ...
                    @if ($stories->nextPageUrl())
                        <a href="{{ $stories->nextPageUrl() }}" class="btn btn-sm btn-white d-sm-block d-none mb-0 ms-auto">Next</a>
                    @else
