  ...
  <div class="col-12 col-xxl-10 mx-auto">
    <div class="card shadow-xs">
      <div class="card-header d-flex align-items-center justify-content-between">
        <div>
          <h5 class="mb-1">
            Planner Idea Details
            <span class="badge badge-key">#{{ $planner->id }}</span>
          </h5>
      ...
      <div class="card-body">
        ...
        <div class="table-responsive">
          <table id="lastModifiedTable" class="table table-sm align-middle mb-4">
            ...
            <tbody>
              @foreach ($planner->getAttributes() as $key => $newValue)
                @php
                    // تحضير القيم الجديدة
                ...
                <tr class="{{ $isDifferent ? 'table-warning' : '' }}">
                  <td class="wrap-cell">
                    <span class="badge badge-key">{{ ucfirst(str_replace('_',' ', $key)) }}</span>
                  </td>
                      ...
                      <tbody>
                        @foreach ($log->new_data as $key => $newVal)
                          @php
                              ...
                              // تحويل العلاقات لأسماء (حسب بيانات نفس اللوج)
                              if ($key === 'user_id') {
...
<script>
  $(function () {
    ...
    $('#exportLastExcel').on('click', function(e){
      ...
      XLSX.writeFile(wb, 'planner_last_modified.xlsx');
    });
    ...
    $('#exportLastPDF').on('click', function(e){
      ...
      doc.text('Planner — Last Modified', 40, 40);
      doc.autoTable({ html:'#lastModifiedTable', startY:60, styles:{fontSize:8} });
      doc.save('planner_last_modified.pdf');
    });
    ...
    $('#exportLogsPDF').on('click', function(e){
      ...
      const doc = new jsPDF('p','pt');
      let y = 40;
      doc.text('Planner — Change Logs (expanded)', 40, y);
      y += 20;
      ...
      setTimeout(function(){
        ...
        doc.save('planner_change_logs.pdf');
