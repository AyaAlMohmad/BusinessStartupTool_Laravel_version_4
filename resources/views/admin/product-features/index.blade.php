        ...
        <div class="card border shadow-xs mb-4">
            <div class="card-header border-bottom pb-0">
                <div class="d-sm-flex align-items-center mb-3">
                    <div>
                        <h6 class="font-weight-semibold text-lg mb-0">Marketing List</h6>
                        <p class="text-sm mb-sm-0">Manage marketing features and notes</p>
                    </div>
                    <div class="ms-auto d-flex">
                        ...
                        <div class="btn-group ms-2">
                            <a href="{{ route('admin.marketing-new.analysis') }}" class="btn btn-sm btn-dark">
            ...
            <div class="card-body px-0 py-0">
                    ...
                    <table class="table align-items-center justify-content-center mb-0">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="text-secondary text-xs font-weight-semibold opacity-7 ps-2">Options</th>
                        ...
                        <tbody>
                            @foreach ($features as $feature)
                                <tr>
                                    <td><p class="text-sm mb-0">{{ $feature->id }}</p></td>
                                    <td><p class="text-sm mb-0">{{ $feature->business->name ?? '-' }}</p></td>
                                    <td><p class="text-sm mb-0">{{ $feature->user->name ?? '-' }}</p></td>
                                    <td><div class="cell-scroll">{!! $feature->formattedPairsFrom('options') !!}</div></td>
                                    <td><div class="cell-scroll">{!! $feature->formattedPairsFrom('notes') !!}</div></td>
                                    <td class="align-middle text-center">
                                        <div class="d-flex justify-content-center">
                                            ...
                                            <a href="{{ route('admin.marketing-new.show', $feature->id) }}"
                                               ...
                                               data-bs-toggle="tooltip" data-bs-title="View Marketing">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <!-- Delete -->
                                            <form action="{{ route('admin.marketing-new.destroy', $feature->id) }}" method="POST" class="m-0">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                                        onclick="return confirm('Are you sure you want to delete this record?')"
                                                        class="btn btn-sm btn-icon-only btn-rounded btn-outline-danger"
                                                        data-bs-toggle="tooltip" data-bs-title="Delete Marketing">
                ...
                <div class="border-top py-3 px-3 d-flex align-items-center">
                    @if ($features->previousPageUrl())
                        <a href="{{ $features->previousPageUrl() }}" class="btn btn-sm btn-white d-sm-block d-none mb-0">Previous</a>
                    @else
                    ...
                    @if ($features->nextPageUrl())
                        <a href="{{ $features->nextPageUrl() }}" class="btn btn-sm btn-white d-sm-block d-none mb-0 ms-auto">Next</a>
                    @else
