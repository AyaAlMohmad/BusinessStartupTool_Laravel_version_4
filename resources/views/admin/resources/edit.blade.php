            ...
            <div class="card border shadow-xs mb-4">
                <div class="card-header border-bottom pb-0">
                    <div class="d-sm-flex align-items-center mb-3">
                        <div>
                            <h6 class="font-weight-semibold text-lg mb-0">Edit Resource</h6>
                            <p class="text-sm mb-sm-0">Edit a resource</p>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <select name="is_global" id="is_global" class="form-control"
                                        ...
                                        <option value="0"
                                            {{ old('is_global', $resource->is_global) == 0 ? 'selected' : '' }}>Regional
                                            Resource</option>
                                        <option value="1"
                                            {{ old('is_global', $resource->is_global) == 1 ? 'selected' : '' }}>Global
                                            Resource</option>
                                    </select>
                                    ...
                                    <select name="region_id" id="region_id" class="form-control">
                                        ...
                                        @foreach ($regions as $region)
                                            <option value="{{ $region->id }}"
                                                {{ old('region_id', $resource->region_id) == $region->id ? 'selected' : '' }}>
                                                {{ $region->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="user_ids[]" id="user_ids" class="form-control" multiple>
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}"
                                                {{ in_array($user->id, old('user_ids', $resource->users->pluck('id')->toArray())) ? 'selected' : '' }}>
