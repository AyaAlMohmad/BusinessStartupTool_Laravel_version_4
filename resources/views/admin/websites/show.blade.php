...
<div class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <h1 class="text-2xl font-bold mb-4">website Idea Details #{{ $website->id }}</h1>
            ...
            <tbody>
                @foreach ($website->getAttributes() as $key => $newValue)
                    ...
                    <tr class="transition-colors @if($isDifferent) bg-yellow-50 hover:bg-yellow-100 @else hover:bg-gray-50 @endif cursor-pointer"
                        ...
                        <td class="border px-4 py-2 font-medium break-words max-w-xs">{{ $fieldNames[$key] ?? $key }}</td>
                ...
                $displayNew = implode(', ', $currentServices);

                $oldServices = $oldData['services'] ?? [];
                if (is_string($oldServices)) {
                    $oldServices = json_decode($oldServices, true) ?? [];
                }
                $displayOld = implode(', ', $oldServices);

                $isDifferent = $displayNew !== $displayOld;
            @endphp

            <tr class="transition-colors @if($isDifferent) bg-yellow-50 hover:bg-yellow-100 @else hover:bg-gray-50 @endif">
                <td class="border px-4 py-2 font-medium">Services / Products</td>
                <td class="border px-4 py-2" style="{{ $isDifferent ? 'background-color:#d1fae5; color:#065f46; font-weight:bold; padding:8px; border-radius:6px;' : '' }}">
                    {{ $displayNew }}
                </td>
                <td class="border px-4 py-2" style="{{ $isDifferent ? 'background-color:#fee2e2; color:#991b1b; font-weight:bold; padding:8px; border-radius:6px;' : '' }}">
                    {{ $displayOld }}
                </td>
            </tr>
            </tbody>
