                                ...
                                <thead class="bg-gray-100">
                                    <tr>
                                        <th class="text-secondary text-xs font-weight-semibold opacity-7">
                                            {{ __('dashboard.item') }}</th>
                                        <th class="text-secondary text-xs font-weight-semibold opacity-7 ps-2">
                                            {{ __('dashboard.cost') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr class="bg-gray-100">
                                        ...
                                        <td>
                                            <p class="text-sm font-weight-bold mb-0">
                                                ${{ number_format($startupCosts->sum('cost'), 2) }}</p>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                                ...
                                <tbody>
                                    @foreach ($fundingSources as $source)
                                        <tr>
                                            ...
                                            <td>
                                                <p class="text-sm font-weight-normal mb-0">
                                                    ${{ number_format($source->amount, 2) }}</p>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <span
                                                    class="text-sm font-weight-normal">{{ $source->expected_date }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                ...
                <div class="card shadow-xs border">
                    <div class="card-header pb-0">
                        <div class="d-sm-flex align-items-center mb-3">
                            <div>
                                <h6 class="font-weight-semibold text-lg mb-0">{{ __('dashboard.financial_overview') }}
                                </h6>
                ...
                <div class="card border shadow-xs mb-4">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                            <div>
                                <h4 class="mb-1 font-weight-bold">Regional Dashboard</h4>
                                {{-- <p class="text-sm text-secondary mb-0">نظرة عامة على المستخدمين والسجلات ضمن مناطق أدوارك --}}
                                </p>
                            </div>
                            <div class="d-flex align-items-center gap-2">
                                <span class="badge bg-dark" style="color:white">
                                    <i class="fas fa-map-marker-alt me-1"></i> My Regions
                                </span>
                            </div>
                        </div>
                        ...
                        @php
                            // $usersInMyRegions و $filledCounts و $securedFunding و $plannedFunding متاحة من الكنترولار
                            $totalRecords = collect($filledCounts ?? [])->sum(function ($i) {
                                return $i['records'] ?? 0;
                            });
                            $securedPct =
                                isset($plannedFunding) && $plannedFunding > 0
                                    ? round(($securedFunding / $plannedFunding) * 100)
                                    : 0;

                            // خريطة أسماء الوحدات بشكل مقروء
                            $labels = [

                                'businessIdeas' => __('dashboard.business_ideas'),
                                'marketResearches' => __('dashboard.market_research'),
                                'marketingNews' => __('dashboard.marketing'),
                                'salesStrategies' => __('dashboard.sales_strategies'),
                                'startSimples' => __('dashboard.start_simple'),
                                'testingIdeas' => __('dashboard.testing_ideas'),
                                'businessSetups' => __('dashboard.business_setups'),
                                'financialPlanners' => __('dashboard.financial_planners'),
                                'websites' => __('dashboard.websites'),
                            ];
                        @endphp
                        ...
                        <div class="row g-3">
                            <div class="col-xl-3 col-sm-6">
                                <div class="card border shadow-xs h-100">
                                    <div class="card-body">
                                        <div
                                            class="icon icon-shape icon-sm bg-dark text-white d-inline-flex align-items-center justify-content-center border-radius-sm mb-3">
                                            <i class="fas fa-users"></i>
                                        </div>
                                        <p class="text-sm text-secondary mb-1">{{ __('dashboard.users_in_my_regions') }}
                                        </p>
                                        <h4 class="mb-0 font-weight-bold">{{ $usersInMyRegions ?? 0 }}</h4>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-3 col-sm-6">
                                <div class="card border shadow-xs h-100">
                                    <div class="card-body">
                                        ...
                                        <p class="text-sm text-secondary mb-1">{{ __('dashboard.total_records') }}</p>
                                        <h4 class="mb-0 font-weight-bold">{{ $totalRecords }}</h4>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-3 col-sm-6">
                                <div class="card border shadow-xs h-100">
                                    <div class="card-body">
                                        ...
                                        <p class="text-sm text-secondary mb-1">{{ __('dashboard.secured_funding') }}</p>
                                        <h4 class="mb-1 font-weight-bold">${{ number_format($securedFunding ?? 0, 2) }}
                                        </h4>
                                        <div class="progress" style="height:7px;">
                                            <div class="progress-bar bg-success" role="progressbar"
                                                style="width: {{ $securedPct }}%;" aria-valuenow="{{ $securedPct }}"
                                                aria-valuemin="0" aria-valuemax="100"></div>
                                        </div>
                                        <small class="text-secondary d-block mt-1">{{ $securedPct }}%
                                            {{ __('dashboard.of_goal') }}</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-3 col-sm-6">
                                <div class="card border shadow-xs h-100">
                                    <div class="card-body">
                                        ...
                                        <p class="text-sm text-secondary mb-1">{{ __('dashboard.planned_funding') }}</p>
                                        <h4 class="mb-0 font-weight-bold">${{ number_format($plannedFunding ?? 0, 2) }}
                                        </h4>
                                    </div>
                                </div>
                            </div>
                        ...
                        {{-- جدول الوحدات داخل مناطقي --}}
                        <div class="card border shadow-xs mt-4">
                            <div class="card-header border-bottom pb-2">
                                <h6 class="font-weight-semibold text-lg mb-0">
                                    {{ __('dashboard.modules_activity_in_my_regions') }}</h6>
                                <p class="text-sm text-secondary mb-0">{{ __('dashboard.project_overview') }}</p>
                            </div>
                            <div class="card-body px-0 py-0">
                                <div class="table-responsive p-0">
                                    <table class="table align-items-center justify-content-center mb-0">
                                        <thead class="bg-gray-100">
                                            <tr>
                                                <th class="text-secondary text-xs font-weight-semibold opacity-7">
                                                    {{ __('dashboard.module') }}</th>
                                                <th class="text-secondary text-xs font-weight-semibold opacity-7 ps-2">
                                                    {{ __('dashboard.records_count') }}</th>
                                                <th class="text-secondary text-xs font-weight-semibold opacity-7 ps-2">
                                                    {{ __('dashboard.distinct_users') }}</th>
                                                <th class="text-secondary text-xs font-weight-semibold opacity-7 ps-2">
                                                    {{ __('dashboard.activity') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach ($filledCounts ?? [] as $key => $counts)
                                                @php
                                                    $label = $labels[$key] ?? \Illuminate\Support\Str::headline($key);
                                                    $records = $counts['records'] ?? 0;
                                                    $users = $counts['users'] ?? 0;
                                                    $pct =
                                                        $totalRecords > 0 ? round(($records / $totalRecords) * 100) : 0;
                                                @endphp
                                                <tr>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <p class="text-sm font-weight-normal mb-0">{{ $records }}</p>
                                                    </td>
                                                    <td>
                                                        <p class="text-sm font-weight-normal mb-0">{{ $users }}</p>
                                                    </td>
                                                    <td style="min-width:180px;">
                                                        ...
                                                        <small
                                                            class="text-secondary d-block mt-1">{{ $pct }}%</small>
                                                    </td>
                                                </tr>
                                            @endforeach

                                            @if (empty($filledCounts) || collect($filledCounts)->isEmpty())
                                                <tr>
                                                    <td colspan="4" class="text-center text-secondary py-4">
                                                        {{ __('No data found for your regions yet.') }}
                                                    </td>
                                                </tr>
                                            @endif
                                        </tbody>
                        ...
                        {{-- مصادر التمويل وتكاليف البدء (داخل مناطقي) --}}
                        <div class="row mt-4">
                            <div class="col-lg-6">
                                <div class="card shadow-xs border h-100">
                                    <div class="card-header border-bottom pb-0">
                                        <h6 class="font-weight-semibold text-lg mb-0">{{ __('dashboard.startup_costs') }}
                                        </h6>
                                        <p class="text-sm mb-2">{{ __('dashboard.cost_details') }}</p>
                                    </div>
                                    <div class="card-body px-0 py-0">
                                                    ...
                                                    @empty
                                                        <tr>
                                                            <td colspan="2" class="text-center text-secondary py-4">
                                                                {{ __('No startup costs found.') }}</td>
                                                        </tr>
                                                    @endforelse
                                                    @if (($startupCosts ?? collect())->count())
                                                        <tr class="bg-gray-100">
                                                            <td>
                                                                <h6 class="mb-0 text-sm font-weight-bold">
                                                                    {{ __('dashboard.total') }}</h6>
                                                            </td>
                                                            <td>
                                                                <p class="text-sm font-weight-bold mb-0">
                                                                    ${{ number_format(($startupCosts ?? collect())->sum('cost'), 2) }}
                                                                </p>
                                                            </td>
                                                        </tr>
                                                    @endif
                            ...
                            <div class="col-lg-6">
                                <div class="card shadow-xs border h-100">
                                    <div class="card-header border-bottom pb-0">
                                        <h6 class="font-weight-semibold text-lg mb-0">
                                            {{ __('dashboard.funding_sources') }}</h6>
                                        <p class="text-sm mb-2">{{ __('dashboard.available_funding') }}</p>
                                    </div>
                                    <div class="card-body px-0 py-0">
                                        <div class="table-responsive p-0">
                                                ...
                                                <tbody>
                                                    @forelse(($fundingSources ?? []) as $source)
                                                        <tr>
                                                            ...
                                                            <td><span
                                                                    class="text-sm font-weight-normal">{{ $source->type }}</span>
                                                            </td>
                                                            <td><span
                                                                    class="text-sm font-weight-normal">{{ $source->expected_date }}</span>
                                                            </td>
                                                    ...
                                                    @empty
                                                        <tr>
                                                            <td colspan="4" class="text-center text-secondary py-4">
                                                                {{ __('No funding sources found.') }}</td>
                                                        </tr>
                                                    @endforelse
                                                </tbody>
                                            </table>
